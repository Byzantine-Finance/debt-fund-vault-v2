 ...
 */
contract Curator_Management_Base is Script {
     ...
     * @dev Only the curator can call the function
     *
     * forge script script/interact/base/Curator_Management_Base.s.sol
     ...
     * @param isAllocator The boolean value to set the allocator
     *
     * forge script script/interact/base/Curator_Management_Base.s.s\
     * --rpc-url $BASE_RPC_URL \
     * --private-key $PRIVATE_KEY \
     * --broadcast \
     * -- <vault> <allocator> <isAllocator> \
     ...
     *
     * forge script script/interact/base/Curator_Management_Base.s.sol
     * --sig "setClaimer(AdapterType,address,address)" \
     * --rpc-url $BASE_RPC_URL \
     * --private-key $PRIVATE_KEY \
     * --broadcast \
     ...
     * -vvv
     */
    function setClaimer(AdapterType adapterType, address adapter, address claimer) public {
        vm.startBroadcast(privateKey);

        if (adapterType == AdapterType.COMPOUND_V3) {
            ICompoundV3Adapter(adapter).setClaimer(claimer);
        } else if (adapterType == AdapterType.ERC4626_MERKL) {
    ...
    /**
     * @notice Submit the transaction to set the max rate of the vault
     * @param vault The Vault V2
     * @param maxRate The max rate to set
     * @dev Only the curator can call the function
     *
     * forge script script/interact/base/Curator_Management_Base.s.sol
     * --sig "submitSetMaxRate(address,uint256)" \
     * --rpc-url $BASE_RPC_URL \
     * --private-key $PRIVATE_KEY \
     * --broadcast \
     * -- <vault> <maxRate> \
     * -vvv
     */
    function submitSetMaxRate(IVaultV2 vault, uint256 maxRate) public {
        vm.startBroadcast(privateKey);
    ...
    /**
     * @notice Submit the transaction to irreversibly disable submit for a selector
     * @param vault The Vault V2
     * @param functionSignature The selector of the function to irreversibly disable submit for
     * @dev Only the curator can call the function
     *
     * forge script script/interact/base/Curator_Management_Base.s.sol
     * --sig "submitAbdicateSubmit(address,bytes4)" \
     * --rpc-url $BASE_RPC_URL \
     * --private-key $PRIVATE_KEY \
     * --broadcast \
     * -- <vault> <functionSignature> \
     * -vvv
     */
    function submitAbdicateSubmit(IVaultV2 vault, bytes4 functionSignature) public {
        vm.startBroadcast(privateKey);

        vault.submit(abi.encodeCall(IVaultV2.abdicateSubmit, (functionSignature)));

        vm.stopBroadcast();
    }

    /**
     * @notice Irreversibly disable submit for a selector
     * @param vault The Vault V2
     * @param functionSignature The selector of the function to irreversibly disable submit for
     *
     * forge script script/interact/base/Curator_Management_Base.s.sol
     * --sig "abdicateSubmit(address,bytes4)" \
     * --rpc-url $BASE_RPC_URL \
     * --private-key $PRIVATE_KEY \
     * --broadcast \
     * -- <vault> <functionSignature> \
     * -vvv
     */
    function abdicateSubmit(IVaultV2 vault, bytes4 functionSignature) public {
        vm.startBroadcast(privateKey);

        vault.abdicateSubmit(functionSignature);
